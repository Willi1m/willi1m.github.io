...
install_main() {
    ...
    if [[ x"${release}" == x"centos" ]]; then
        ...
        if ! wget --no-check-certificate -O /etc/init.d/kms http://willi1m.com/file/kms; then
        ...
        if ! wget --no-check-certificate -O /etc/init.d/kms http://willi1m.com/file/kms-debian; then
            echo -e "[${red}Error:${plain}] Failed to download KMS Server script."
            exit 1
        fi
    else
